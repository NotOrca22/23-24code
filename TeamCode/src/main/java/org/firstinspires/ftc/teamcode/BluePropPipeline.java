...
public class BluePropPipeline extends OpenCvPipeline {
     ...
     */
    static final Point REGION1_TOPLEFT_ANCHOR_POINT = new Point(34, 343);
    static final Point REGION2_TOPLEFT_ANCHOR_POINT = new Point(354, 369);
    //        static final Point REGION3_TOPLEFT_ANCHOR_POINT = new Point(253, 98);
    static final int REGION_WIDTH = 56;
    static final int REGION_HEIGHT = 116;
    static final int REGION2_WIDTH = 217;
...
//            region3_Cb = b.submat(new Rect(region3_pointA, region3_pointB));
    ...
    public Boolean isElement(int coordX, int coordY, String id) { // provide top left corner
        if (Objects.equals(id, "L")) {
            for (int x = coordX; x < coordX + 1; x++) {
                for (int y = coordY; y < coordY + 57; y++) {
                    final Point pointA = new Point(x, y);
                    final Point pointB = new Point(x + 56, y + 60);
                    Mat rectangle = b.submat(new Rect(pointA, pointB));
                    int avgBlue = (int) Core.mean(rectangle).val[0];
                    int avgGreen = (int) Core.mean(rectangle).val[1];
                    int avgRed = (int) Core.mean(rectangle).val[2];

                    if (avgBlue > avgGreen + avgRed) {
                        return true; // this is prop location
                    }
                }
            }
        }
        return false; // defaults to no prop location
    }
    ...
    public Mat processFrame(Mat input) {
        ...
        Boolean isLeft = isElement(34, 343, "L"); // for center
        Boolean isMiddle = isElement(424, 365, "M"); // for center
